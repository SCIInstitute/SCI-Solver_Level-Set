                              ...
                              IdxVector_d& part_label_d, IdxVector_d& block_xadj, IdxVector_d& block_adjncy)
{
  ...
  while (numActive > 0)
  {
    ...
    if (h_ActiveListNew.size() > 0)
    {
      ...
      int sharedSize = sizeof (LevelsetValueType)* 4 * largest_ele_part + sizeof (short) *largest_vert_part*largest_num_inside_mem;
  ...
  printf("Size of new narrowband is %d\n", numb);
  ...
  //	}
  //	else 
  //		printf("nblocks for kernel_recover_Tsign is 0!!\n");
